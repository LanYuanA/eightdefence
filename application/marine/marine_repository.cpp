...
std::vector<Binding> defaultBindings() {
    ...
    return {{"VENT-01", "MOTOR-02", "RS485", "0x02", {"ventilation", "speed", "status"}, false, 1, timestamp}, {"COOL-01", "MOTOR-0E", "RS485", "0x0E", {"cooling", "status"}, false, 1, timestamp}, {"WATER-01", "MOTOR-0E", "RS485", "0x0E", {"water", "status"}, false, 1, timestamp}, {"DRAIN-01", "MOTOR-0F", "RS485", "0x0F", {"drainage", "status"}, false, 1, timestamp},
        {"EXHAUST-FAN-01", "MOTOR-02", "RS485", "0x02", {"motor-speed", "direction", "status"}, true, 1, timestamp}, {"FIRE-PUMP-01", "MOTOR-0E", "RS485", "0x0E", {"motor-speed", "direction", "status"}, true, 1, timestamp}, {"DRAIN-PUMP-01", "MOTOR-0F", "RS485", "0x0F", {"motor-speed", "direction", "status"}, true, 1, timestamp}};
}
}
...
std::string MarineRepository::readBindings() { const fs::path path = fs::path(root_) / "bindings.json"; if (!fs::exists(path)) { bindings_ = defaultBindings(); return writeBindings(); } std::string text, error; if (!(error = readFile(path, text)).empty()) return error; JsonValue root; if (!parseJson(text, root, error) || !root.isArray()) return "bindings.json 格式错误。"; bindings_.clear(); for (const auto& item : root.asArray()) { Binding binding; if (!bindingFromJson(item, binding)) return "bindings.json 包含无效绑定。"; bindings_.push_back(std::move(binding)); } bool changed = false; for (const auto& expected : defaultBindings()) if (std::none_of(bindings_.begin(), bindings_.end(), [&](const Binding& current) { return current.logicalExecutorId == expected.logicalExecutorId; })) { bindings_.push_back(expected); changed = true; } return changed ? writeBindings() : ""; }
...
std::string nodeStatusToString(NodeStatus status) { switch (status) { case NodeStatus::Waiting: return "waiting"; case NodeStatus::Running: return "running"; case NodeStatus::Completed: return "completed"; case NodeStatus::Cancelled: return "cancelled"; case NodeStatus::Failed: return "failed"; } return "waiting"; }
