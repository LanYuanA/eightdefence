...
// 模拟原版 Java 的 "IndoorAirQualityTransmitterModel" 数据保存行为
// 以及 "ChartDataUtils" 的除 10 或入图表功能
static void cloud_process_data_internal(const uint8_t *resp, size_t resp_len, cloud_sensor_type_t sensor_type) {
    if (resp_len >= 7 && resp[0] == DEV_CLOUD_ADDR && resp[1] == 0x03) {
        ...
        switch(sensor_type) {
            case CLOUD_TYPE_TEMPERATURE: {
                ...
                g_sensor_data.temp = temp_actual;
...
void cloud_process_pm25(const uint8_t *resp, size_t resp_len)       { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_PM25); }
void cloud_process_pm10(const uint8_t *resp, size_t resp_len)       { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_PM10); }
void cloud_process_humidity(const uint8_t *resp, size_t resp_len)   { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_HUMIDITY); }
void cloud_process_temperature(const uint8_t *resp, size_t resp_len){ cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_TEMPERATURE); }
void cloud_process_tvoc(const uint8_t *resp, size_t resp_len)       { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_TVOC); }
void cloud_process_ch2o(const uint8_t *resp, size_t resp_len)       { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_CH2O); }
void cloud_process_o3(const uint8_t *resp, size_t resp_len)         { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_O3); }
void cloud_process_co2(const uint8_t *resp, size_t resp_len)        { cloud_process_data_internal(resp, resp_len, CLOUD_TYPE_CO2); }
