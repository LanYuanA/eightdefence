...
<template>
  <section class="panel application-flow-panel">
    ...
    <div class="application-flow-map" :class="[`run-${status}`]">
      <svg class="flow-particle-network" viewBox="0 0 1200 650" preserveAspectRatio="none" aria-hidden="true">
        <defs>
        ...
        <path id="sensor-route-a" class="data-route" d="M180 615 C160 490 215 425 350 340 S515 155 590 85"/>
        <path id="sensor-route-b" class="data-route" d="M360 615 C335 490 370 425 465 340 S545 155 590 85"/>
        <path id="control-route-a" class="control-route" d="M610 95 C690 170 740 260 770 350 S835 500 900 615"/>
        <path id="control-route-b" class="control-route" d="M635 95 C780 190 875 280 930 370 S970 510 1040 615"/>
        <g v-if="status === 'running'" filter="url(#flow-glow)">
          <circle v-for="delay in ['0s','-1.6s','-3.2s']" :key="`data-a-${delay}`" class="flow-particle data-particle" r="5"><animateMotion dur="4.8s" :begin="delay" repeatCount="indefinite"><mpath href="#sensor-route-a"/></animateMotion></circle>
          <circle v-for="delay in ['-0.8s','-2.4s','-4s']" :key="`data-b-${delay}`" class="flow-particle data-particle" r="4"><animateMotion dur="4.8s" :begin="delay" repeatCount="indefinite"><mpath href="#sensor-route-b"/></animateMotion></circle>
          <circle v-for="delay in ['0s','-1.4s','-2.8s']" :key="`control-a-${delay}`" class="flow-particle control-particle" r="5"><animateMotion dur="4.2s" :begin="delay" repeatCount="indefinite"><mpath href="#control-route-a"/></animateMotion></circle>
          <circle v-for="delay in ['-.7s','-2.1s','-3.5s']" :key="`control-b-${delay}`" class="flow-particle control-particle" r="4"><animateMotion dur="4.2s" :begin="delay" repeatCount="indefinite"><mpath href="#control-route-b"/></animateMotion></circle>
        </g>
      </svg>
...
.flow-particle-network{position:absolute;z-index:1;inset:0;width:100%;height:100%;pointer-events:none}.data-route,.control-route{fill:none;stroke-width:2;stroke-dasharray:7 11;opacity:.55}.data-route{stroke:var(--flow-data)}.control-route{stroke:var(--flow-control)}.flow-particle{filter:drop-shadow(0 0 5px currentColor)}.data-particle{fill:var(--flow-data);color:var(--flow-data)}.control-particle{fill:var(--flow-control);color:var(--flow-control)}.application-flow-panel{padding:30px 32px}.application-flow-heading h2{font-size:28px}.application-flow-legend{font-size:15px;gap:22px}.application-flow-map{min-width:1000px;padding:0 70px}.auto-flow-layer{z-index:2;grid-template-columns:48px 165px 1fr;gap:14px;padding:16px 18px;border-radius:11px;background:linear-gradient(120deg,rgba(20,43,59,.91),rgba(10,27,40,.84))}.auto-layer-index{width:40px;height:36px;font-size:14px}.auto-layer-title small{font-size:11px}.auto-layer-title strong{font-size:20px}.current-application-node{max-width:780px;padding:13px 16px}.current-application-node>span{width:43px;height:43px;font-size:13px}.current-application-node h3{font-size:22px}.current-application-node p,.current-application-node small,.current-application-node em{font-size:13px}.auto-layer-bridge{z-index:2;height:34px;font-size:14px}.auto-layer-bridge b{font-size:18px}.auto-flow-channels{gap:15px}.auto-flow-channel,.physical-group{padding:11px 13px}.auto-flow-channel>header,.physical-group>header{font-size:14px}.auto-flow-channel>header small,.physical-group>header small{font-size:12px}.auto-node-grid{grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px}.auto-service-node{gap:9px;padding:10px 11px;border-left-width:3px}.auto-service-node>b{width:33px;height:31px;font-size:12px}.auto-service-node strong,.auto-endpoint-node strong,.sensor-chip-grid strong,.hardware-chip-grid strong{font-size:15px}.auto-service-node small,.auto-endpoint-node small,.sensor-chip-grid small,.hardware-chip-grid small{font-size:12px}.awareness-bypass{padding:12px 14px}.awareness-bypass>span{font-size:12px}.awareness-bypass strong{font-size:15px}.awareness-bypass small{font-size:12px}.abstraction-node-grid{grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px}.auto-endpoint-node{padding:11px}.auto-endpoint-node>b{font-size:12px}.physical-group{min-height:75px}.sensor-chip-grid,.hardware-chip-grid{gap:8px}.sensor-chip-grid article,.hardware-chip-grid article{min-width:135px;padding:9px 10px}.flow-column-label{font-size:11px}.vertical-flow{width:3px}.vertical-flow i{width:10px;height:10px}.application-flow-summary{margin-top:15px;padding-top:14px;font-size:15px}.application-flow-summary b{font-size:19px}.empty-flow-side{font-size:13px}@media(max-width:1150px){.application-flow-heading,.application-flow-map,.application-flow-summary{min-width:1000px}}
</style>
