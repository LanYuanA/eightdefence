...
<IndependentApplication v-if="detailOpen && activeSession" :key="activeSession.id" :blocked="gatewayLost" :app="activeSession.app" :run="run" :snapshot="sensorSnapshot" :gateway="gatewayConnected" :motors="motors" :motor-mode="motorMode" :emergency-stopped="emergencyStopped" :notice="notice"
      ...
      <footer class="marine-footer"><span>SD VESSEL <i/> 民用船舶软件定义演示</span><span>软软解耦 · 业务服务自由组合</span><span><RouterLink to="/software-defined-platform">任务解耦演示 ↗</RouterLink><RouterLink to="/hardware-decoupling">硬件替换演示 ↗</RouterLink><RouterLink to="/legacy-dashboard">系统总图 ↗</RouterLink></span></footer>
    </main>
